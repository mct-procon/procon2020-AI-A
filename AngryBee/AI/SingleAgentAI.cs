...
namespace AngryBee.AI
{
    public class SingleAgentAI : MCTProcon31Protocol.AIFramework.AIBase
    {
        ...
        protected override void Solve()
        {
            ...
            for (; deepness <= maxDepth; deepness++)
            {
                ...
                for (int agent = 0; agent < MyAgentsCount; ++agent)
                {
                    ...
                    NegaMax(deepness, state, int.MinValue + 1, 0, evaluator, null, nextways, agent, deepness, MyAgentsCount, EnemyAgentsCount);
                }
                ...
                for (int i = 0; i < MyAgentsCount; ++i)
                {
                    ...
                    for (int agent = 0; agent < MyAgentsCount; ++agent)
                    {
                        ...
                        NegaMax(deepness, state, int.MinValue + 1, 0, evaluator, best1, nextways, agent, deepness, MyAgentsCount, EnemyAgentsCount);
                    }
        ...
        //NegaMaxではない
        private int NegaMax(int deepness, SearchState state, int alpha, int count, PointEvaluator.Base evaluator, Decision ngMove, Unsafe16Array<Way> nextways, int nowAgent, int watch_deepness, int MyAgentsCount, int EnemyAgentsCount)
        {
            ...
            foreach (var way in ways.Data[nowAgent])
            {
                ...
                state = state.GetNextState(MyAgentsCount, newways);

                int res = NegaMax(deepness - 1, state, alpha, count + 1, evaluator, ngMove, nextways, nowAgent, watch_deepness, EnemyAgentsCount, MyAgentsCount);
                if (alpha < res)
                {
